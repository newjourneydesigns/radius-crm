  ...
  const loadEvents = useCallback(
    async (opts: { force?: boolean } = {}) => {
      ...
      try {
        ...
        const res = await fetch(`/api/circle-leader-toolkit/events/${qs}`, { cache: 'no-store' });
        if (res.status === 401) {
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="mt-8 text-center space-y-1.5">
          <p className="text-xs text-neutral-400">
            New to the app or need a hand?{' '}
            <Link href={`/circle-leader-toolkit/${groupId}/help`} className="cs-footer-link font-semibold">
              View the help guide
            </Link>
          </p>
          <p className="text-xs text-neutral-400">
