...
export default function CircleSummaryNotificationSettingsPage() {
  ...
  async function loadSettings() {
    ...
    setError(null);
    try {
      const res = await fetch('/api/circle-summary/notifications/', { cache: 'no-store' });
  ...
  async function savePreference(key: keyof Preferences, value: boolean) {
    ...
    setError(null);
    try {
      const res = await fetch('/api/circle-summary/notifications/', {
        method: 'POST',
      ...
      if (key === 'badge_count_enabled') {
        const counts = await fetch('/api/circle-summary/alerts/', { cache: 'no-store' }).then((r) => r.json()).catch(() => null);
  ...
  async function disableCurrentDevice() {
    ...
    try {
      ...
      const subscription = await registration.pushManager.getSubscription();
      if (subscription) {
        await fetch(`/api/circle-summary/notifications/?endpoint=${encodeURIComponent(subscription.endpoint)}`, { method: 'DELETE' });
