...
export default function CircleOnboardingPrompts({ groupId }: { groupId: string }) {
  ...
  async function enableNotifications() {
    ...
    try {
      ...
      await fetch('/api/circle-summary/notifications/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
function LinkLikeSettings({ groupId }: { groupId: string }) {
  return (
    <Link href={`/circle-summary/${groupId}/settings`} className="rounded-full px-3 py-2 text-xs font-bold text-neutral-500 hover:bg-neutral-100">
      Settings
    </Link>
  );
}
