...
function SettingsButton({ groupId, active }: { groupId: string; active: ActiveTab | null }) {
  ...
  return (
    <Link
      href={`/circle-summary/${groupId}/settings`}
      aria-label="Settings"
      aria-current={isActive ? 'page' : undefined}
      className={
        'absolute -top-4 right-0 sm:-top-6 inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 ' +
        'text-xs font-semibold uppercase tracking-wide transition-colors backdrop-blur-sm ' +
        (isActive
