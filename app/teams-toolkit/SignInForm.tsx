...
export default function SignInForm() {
  ...
  async function requestCode(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch('/api/teams-toolkit/auth/request-code', {
        method: 'POST',
  ...
  async function verifyCode(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch('/api/teams-toolkit/auth/verify-code', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      {stage === 'identifier' && (
        <>
          ...
          <p className="text-sm text-neutral-600 mb-5">
            Enter the email on your team leader profile.
          </p>
          ...
          <form onSubmit={requestCode} className="space-y-4">
            ...
            <p className="text-xs text-neutral-500 text-center pt-1">
              Need help? Contact your director.
            </p>
          </form>
        </>
      )}
