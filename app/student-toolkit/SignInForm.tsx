...
export default function SignInForm({
  ...
  async function requestCode(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch('/api/student-toolkit/auth/request-code/', {
        method: 'POST',
  ...
  async function verifyCode(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch('/api/student-toolkit/auth/verify-code/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const path = `${STUDENT_TOOLKIT_PREFIX}/${encodeURIComponent(String(data.leaderId))}/home/`;
      router.replace(cleanHost ? stripStudentToolkitPrefix(path) : path);
    } catch {
      setError('Network error — please try again.');
    } finally {
      setSubmitting(false);
      ...
      {stage === 'identifier' && (
        <>
          ...
          <p className="text-sm text-neutral-600 mb-5">
            Enter the email on your student leader profile.
          </p>
          ...
          <form onSubmit={requestCode} className="space-y-4">
            ...
            <p className="text-xs text-neutral-500 text-center pt-1">
              No email on your profile? Ask your student pastor to add one.
            </p>
          </form>
        </>
      )}
