/**
 * GET /api/teams-toolkit/inbox  — the signed-in team leader's inbox messages.
 * POST /api/teams-toolkit/inbox — marks a message recipient row read.
 *
 * Reuses the shared `circle_summary_inbox_*` tables (keyed by leader_id), so
 * team leaders receive inbox messages through the same admin tooling.
 */
...
export async function GET() {
  ...
  if (messageError && (isMissingColumn(messageError, 'status') || isMissingColumn(messageError, 'category'))) {
