...
export default function CircleLeaderProfilePage() {
  ...
  // Which proposed fields to actually write. CCB is authoritative for the meeting
  // details but not for everything — a circle led by a couple keeps its RADIUS
  // name, not the one main leader CCB has on the group.
  ...
  const handleResyncCcb = async () => {
    ...
    try {
      ...
      setResyncSelected(new Set(json.changes.map((c: { field: string }) => c.field)));
    } catch (err: any) {
  ...
  const applyResync = async () => {
    if (!resyncPreview || resyncSelected.size === 0) return;
    setIsResyncing(true);
    try {
      const apply = Object.fromEntries(
        Object.entries(resyncPreview.values).filter(([field]) => resyncSelected.has(field))
      );
      const { data: sess } = await supabase.auth.getSession();
      const token = sess?.session?.access_token;
      const res = await fetch(`/api/circle-leaders/${leaderId}/resync-ccb`, {
  ...
  return (
    <ProtectedRoute>
                  ...
                  {!isHostTeam && (isEditing || leader.ccb_group_name) && <div>
                    ...
                    <dd className="mt-1">
                      {isEditing ? (
                        ...
                        <span className="text-sm text-slate-200">{leader.ccb_group_name}</span>
                      )}
                    </dd>
                  </div>}
      ...
      {resyncPreview && (
        <ResyncPreviewModal
          changes={resyncPreview.changes}
          selected={resyncSelected}
          isApplying={isResyncing}
          onToggle={(field) =>
          ...
          onCancel={() => setResyncPreview(null)}
          onApply={applyResync}
        />
      )}
