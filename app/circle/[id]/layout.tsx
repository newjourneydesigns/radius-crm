...
const TABS = [
  ...
  // { label: 'Scorecard', route: (id: string) => `/circle/${id}/scorecard`,     circleOnly: true }, // hidden — not ready for ACPDs
  // { label: 'Snapshot',  route: (id: string) => `/circle/${id}/leadership-snapshot`, adminOnly: true, circleOnly: true }, // hidden — not ready for ACPDs
    ...
    <>
      <div className="sticky top-0 z-40 bg-vc-submenu backdrop-blur-sm border-b border-vc-900/60">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Mobile: dropdown (keeps the row tidy on small screens) */}
          <div className="lg:hidden py-2">
            <div className="relative">
              <button
                ...
                className="flex w-full items-center justify-between gap-2 rounded-lg border border-vc-900/60 bg-black/15 px-3.5 py-2.5 text-sm font-semibold text-white hover:bg-black/25 transition-colors"
              >
              ...
              {navOpen && (
                <>
                  ...
                  <div
                    role="menu"
                    className="absolute left-0 mt-1.5 z-50 w-full overflow-hidden rounded-xl border border-vc-900/60 bg-vc-submenu shadow-xl ring-1 ring-black/20 py-1"
                  >
          ...
          {/* Desktop: original tab row */}
          <nav className="hidden lg:flex" aria-label="Section navigation">
            {filteredTabs.map(tab => {
              const href = tab.route(id);
              return (
