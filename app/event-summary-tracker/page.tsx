...
import AiSummaryMarkdown from '../../components/ai/AiSummaryMarkdown';
      ...
      <div className="space-y-5">
        ...
        {rosterAdds.length > 0 && (
          <RosterAddSection
            key={row.leader.id}
            requests={rosterAdds}
            onToggle={onToggleRosterAdd}
            ccbGroupId={row.leader.ccb_group_id ?? null}
          />
        )}
...
function RosterAddSection({
  ...
  /** Circle's CCB group id — powers the Open CCB shortcut; hidden when unknown. */
      ...
      <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-2">
        ...
        {ccbGroupId && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 shrink-0 rounded-lg border border-amber-500/40 px-2.5 py-1 text-xs font-medium text-amber-200 hover:bg-amber-500/20 transition-colors"
          >
      ...
      <ul className="mt-3 space-y-2">
        {requests.map((p, i) => {
          ...
          return (
            ...
            >
              <div className="min-w-0">
                <CopyTextButton value={name} className="text-sm font-medium text-slate-100" />
                {(p.phone || p.email) && (
                  <div className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs">
                    {p.phone && (
                      <CopyTextButton
                        value={p.phone}
                        copyValue={p.phone.replace(/\D/g, '')}
                        className="text-vc-300 hover:text-vc-200"
                      />
                    )}
                    {p.email && (
                      <CopyTextButton value={p.email} className="text-vc-300 hover:text-vc-200" />
                    )}
                  </div>
                )}
                {added && (
