...
-- This script addresses all the secu-- Circle Types -- Connection Types policies
CREA-- Frequencies p-- Statuses policies
...
CREATE POLICY "acpd_users_write_statuses" ON statuses
  ...
  WITH CHECK (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'ACPD'));CREATE POLICY "authenticated_users_read_frequencies" ON frequencies
  FOR SELECT TO authenticated USING (true);

CREATE POLICY "acpd_users_write_frequencies" ON frequencies
  FOR ALL TO authenticated 
  USING (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'ACPD'))
  WITH CHECK (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'ACPD'));Y "authenticated_users_read_connection_types" ON connection_types
  FOR SELECT TO authenticated USING (true);
...
CREATE POLICY "acpd_users_write_circle_types" ON circle_types
  FOR ALL TO authenticated 
  USING (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'ACPD'))
  WITH CHECK (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'ACPD'));nings from Supabase linter
...
CREATE POLICY "admin_users_write_circle_types" ON circle_types
  FOR ALL TO authenticated 
...
CREATE POLICY "admin_users_write_connection_types" ON connection_types
  FOR ALL TO authenticated 
  USING (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin'))
  WITH CHECK (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin'));
...
CREATE POLICY "admin_users_write_frequencies" ON frequencies
  FOR ALL TO authenticated 
  USING (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin'))
  WITH CHECK (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin'));
...
CREATE POLICY "admin_users_write_statuses" ON statuses
  FOR ALL TO authenticated 
  USING (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin'))
  WITH CHECK (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin'));
