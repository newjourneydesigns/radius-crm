...
| `LEADER_SESSION_SECRET` | HMAC secret (min 32 chars) signing leader magic-link/session tokens. **Must be byte-identical on both Netlify sites** — signed on main, verified on toolkit. | Yes |
| `LEADER_TOOLKIT_HOST` | Dedicated toolkit hostname (e.g. `circlestoolkit.netlify.app`). When set, middleware serves the toolkit at this host's root and magic links point here. Set on **both** sites. | No |
| `DISABLE_SCHEDULED_FUNCTIONS` | Set `true` on the toolkit site so cron runs only on main (no duplicate reminders/syncs); leave unset on main. Toolkit also auto-disables cron when its URL matches `LEADER_TOOLKIT_HOST`. | No |
| `LEADER_TOOLKIT_RESEND_API_KEY` | Resend key for toolkit emails (separate from `RESEND_API_KEY`); falls back to `RESEND_API_KEY`. | Yes |
| `LEADER_TOOLKIT_EMAIL_FROM` / `LEADER_TOOLKIT_EMAIL_FROM_NAME` | Sender address/name for toolkit emails; falls back to `EMAIL_FROM`/`EMAIL_FROM_NAME`. | No |
| `NEXT_PUBLIC_WEB_PUSH_VAPID_PUBLIC_KEY` | VAPID public key for Web Push — shared by the toolkit and Today-page reminders. | No |
