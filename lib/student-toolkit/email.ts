/**
 * Sends the 6-digit sign-in code to a student leader's email via Resend.
 * Mirrors lib/teams-toolkit/email.ts with Student Toolkit branding.
 */
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:560px;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 2px rgba(0,0,0,0.05);">
          <tr>
            <td style="background:${VCC_GREEN};padding:36px 28px;text-align:center;">
              <div style="font-family:${FONT_STACK};font-weight:800;font-style:italic;text-transform:uppercase;color:#ffffff;font-size:32px;line-height:1.05;letter-spacing:0.5px;">
                ${escapeHtml(heroTitle)}
              </div>
              ${heroSubtitle ? `<div style="margin-top:10px;color:rgba(255,255,255,0.92);font-size:15px;font-weight:500;line-height:1.4;">${escapeHtml(heroSubtitle)}</div>` : ''}
            </td>
          </tr>
          <tr>
            <td style="padding:28px 28px 24px 28px;color:${VCC_INK};font-size:16px;line-height:1.55;">
              ${bodyHtml}
            </td>
          </tr>
        </table>
...
async function sendBrandedEmail(opts: {
  ...
  // Falls back through the other two toolkits' keys, then RADIUS's internal one,
  // so the student portal works before it has a Resend account of its own.
  const resendApiKey =
    process.env.STUDENT_TOOLKIT_RESEND_API_KEY ||
  ...
  const fromEmail =
    process.env.STUDENT_TOOLKIT_EMAIL_FROM ||
    process.env.LEADER_TOOLKIT_EMAIL_FROM ||
    process.env.EMAIL_FROM ||
    'onboarding@resend.dev';
  const fromName =
    process.env.STUDENT_TOOLKIT_EMAIL_FROM_NAME ||
...
export async function sendOtpEmail(opts: {
  ...
  const html = emailShell({
    preheader: `Your 6-digit Student Toolkit sign-in code: ${code}`,
    heroTitle: 'Student Toolkit',
    heroSubtitle: 'Your sign-in code',
    bodyHtml:
      `<p style="margin:0 0 12px 0;">Hi ${escapeHtml(leaderName.split(' ')[0] || 'there')},</p>` +
      `<p style="margin:0 0 8px 0;">Enter this code to sign in to the Student Toolkit:</p>` +
      codeBlock +
  ...
  return sendBrandedEmail({ to, subject: `Your Student Toolkit code: ${code}`, html });
}
