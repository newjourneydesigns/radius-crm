 ...
 *
 * The Circle Leader Toolkit, the Teams Toolkit and the Student Leader Toolkit are
 * each deployed as their own Netlify site from this same repo, so they would otherwise register and run
 ...
 * STUDENT_TOOLKIT_HOST aren't set on it (the middleware already serves those hosts via the same
...
const DEFAULT_STUDENT_TOOLKIT_HOST = 'studentstoolkit.netlify.app';
...
export function scheduledFunctionsDisabled(): boolean {
  ...
  const toolkitHosts = [
    ...
    process.env.STUDENT_TOOLKIT_HOST || DEFAULT_STUDENT_TOOLKIT_HOST,
  ];
