...
-- 1. Update the user creation function to handle conflicts better
...
-- 2. Create a function to bootstrap user profiles for existing auth users
...
-- 3. Run the bootstrap function to create missing user profiles
SELECT public.bootstrap_user_profiles();

-- 4. Manually create your user profile with ACPD role
INSERT INTO public.users (id, email, name, role)
VALUES (
...
-- 5. Fix the RLS policies to handle cases where user profile doesn't exist yet
...
-- 6. Update reference table policies to be more graceful
-- ACPD List
