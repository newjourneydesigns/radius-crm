...
/**
 * Admin editor for a single Resources HTML doc. The same component backs both the
 * Circle Leader and Host Team resources pages — the `audience` prop selects which
 * doc is loaded/saved (see /api/admin/circle-leader-resources).
 */
export default function LeaderResourcesAdmin({ audience, title, audienceLabel }: Props) {
  ...
  const loadLatest = useCallback(async (tok: string, opts: { showSpinner?: boolean } = {}) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/circle-leader-resources?audience=${audience}&t=${Date.now()}`, {
  ...
  async function save(): Promise<boolean> {
    ...
    try {
      const res = await fetch('/api/admin/circle-leader-resources', {
        ...
        body: JSON.stringify({ body_html: bodyHtml, audience }),
      });
      const data = await res.json();
        ...
        <div className="mb-6 flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">
              The content below appears to {audienceLabel} on the Resources tab of their toolkit.
