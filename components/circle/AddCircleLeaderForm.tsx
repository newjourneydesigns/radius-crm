...
/**
 * Creates a Circle Leader by hand, for circles that don't exist in CCB yet —
 * invited or pipeline leaders, plants, anything the CCB group import can't reach.
 * A circle that already has a CCB group should go through Import from CCB instead,
 * which pulls its details and links its calendar events.
 */
export default function AddCircleLeaderForm({
  accessToken,
  referenceData,
}: {
  ...
  const [existingNames, setExistingNames] = useState<{ name: string; circle_name: string | null }[]>([]);

  // Existing names power the duplicate warning below. The CCB import refuses to
  // import the same group twice; a manual add has no such guard, so at least say
  // something before a second "Casey Bates" lands in the list.
  useEffect(() => {
    let cancelled = false;
    supabase
      .from('circle_leaders')
      .select('name, circle_name')
      .then(({ data, error: fetchError }) => {
        ...
        setExistingNames((data as { name: string; circle_name: string | null }[]) || []);
      });
    return () => {
      cancelled = true;
    };
  ...
  const duplicate = useMemo(() => {
    ...
    return (
      existingNames.find(
        (l) => l.name?.trim().toLowerCase() === needle || l.circle_name?.trim().toLowerCase() === needle
      ) || null
    );
  }, [trimmedName, existingNames]);

  const showStartDate = /(bi-?week|every other)/i.test(values.frequency);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setValues(EMPTY_FORM);
      setExistingNames((prev) => [...prev, { name: data.name, circle_name: data.circle_name ?? null }]);
    } catch (err: any) {
      ...
      <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <div className="rounded-md bg-green-50 dark:bg-green-900/30 p-4">
          <p className="text-sm text-green-700 dark:text-green-400">
            <strong>{created.name}</strong> was added as a new circle leader.
          </p>
        </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <h2 className="text-lg font-medium text-gray-900 dark:text-white mb-1">Add a Circle Leader</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
          ...
          <strong>Import from CCB</strong> instead so its details and calendar events come across too.
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
            <Field label="Leader Name" htmlFor="leader-name" required>
              <input
                ...
                placeholder="e.g. Casey and Ashley Bates"
          ...
          <Field label="Director (ACPD)" htmlFor="leader-acpd">
            <select
              id="leader-acpd"
              value={values.acpd}
              onChange={(e) => setField('acpd', e.target.value)}
              className={inputClass}
            >
      ...
      <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Status" htmlFor="circle-status">
            <select
              id="circle-status"
              value={values.status}
      ...
      <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <SectionHeading
          title="CCB (optional)"
          description="Only if this leader or circle already exists in CCB. These record the link — they don't pull anything across."
        />
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field
            label="CCB Group ID"
