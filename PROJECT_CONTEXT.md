...
26. **CCB Person Search** — Reusable search component (`CCBPersonLookup.tsx`) for looking up individuals in CCB by name or phone. Used across ConnectPersonModal, Circle Leader edit form, and the Person Lookup page. API route: `POST /api/ccb/person-search`. CCB client method: `searchIndividuals()` in `lib/ccb/ccb-client.ts`.
